...
func (h *handlers) listRestorePresetsHandler(c echo.Context) error {
	...
	if h.servs.RbacService.Enabled() {
		catalog = filterRestoreCatalog(catalog, access.CanViewPreset, access.CanExecutePreset)
	} else {
		catalog = filterRestoreCatalog(catalog, func(string) bool { return true }, func(string) bool { return true })
	}
...
func (h *handlers) getRestoreBackupDownloadHandler(c echo.Context) error {
	...
	presetTitle := auditPresetTitle(id)
	src := auditSource(c)
...
func auditSource(c echo.Context) string {
...
func auditPresetTitle(presetID string) string {
